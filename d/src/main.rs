...
//! 35      3      Reserved (must be zero, ignored on read for forward compat)
...
//! ```
//!
//! Each encrypted chunk is: XChaCha20Poly1305( plaintext_chunk , nonce=derived, AAD=chunk_num_le )
...
//! - Each chunk is encrypted/authenticated independently with its own nonce + AAD containing chunk_num.
//! - This provides: chunk integrity, order integrity (reorder fails), truncation detection (via header size + trailing check).
//!
...
//! - Derived key and password are zeroized after use (best-effort on all paths).
...
//! - Header is not encrypted (salt, params, size, nonce_prefix are public metadata).
...
//! - Per-chunk independent auth + nonce binding => robust against partial tampering, truncation, reorder.
//! - Argon2id + XChaCha20Poly1305 => modern, audited, recommended primitives. No custom crypto.
//! - Atomic temp + sync + rename => durability + no partial outputs on crash/disk full/power loss.
...
// ============================================================================
// CRYPTO: Key Derivation & Nonce Construction
// ============================================================================
...
/// could cause DoS/OOM, non-zero reserved in strict mode, etc.) causes an
...
fn read_header<R: Read>(reader: &mut R) -> Result<FileHeader> {
    ...
    // We ignore reserved bytes for forward compatibility; future versions may use them.
